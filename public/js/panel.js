...
async function _fillTrains(selHour) {
  ...
  if (!trains || trains.length === 0) return; // stays hidden — graceful degrade
  ...
  if (lp && lpSlot) {
    lpSlot.innerHTML = ` <span class="rp-lp-chip" title="Long-period groundswell in the mix">LP ${lp.period.toFixed(0)}s</span>`;
  }
}
